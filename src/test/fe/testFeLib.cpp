...
static bool checkRefIntegration(const size_t &n, const size_t &i,
  ...
  if (std::abs(I_exact - I_approx) > tol) {
    ...
    std::cout << "Print " << i << " " << j << " " << k
              << " debug id = " << debug_id << "\n";
    for (auto qd : qds) std::cout << qd.printStr() << "\n";
...
void test::testTetElem(size_t n) {
    ...
    for (size_t i = 0; i <= n; i++)
      for (size_t j = 0; j <= n; j++)
        for (size_t k = 0; k <= n; k++) {
          ...
          //
          nodes = {util::Point3(), util::Point3(1., 0., 0.),
                   util::Point3(0., 1., 0.), util::Point3(0., 0., 1.)};
          qds = quad.getQuadPoints(nodes);
          // test integration of polynomial f(s,t) = s^i t^j
          // get the exact integration
          debug_id = 0;
          ...
          //
          nodes = {util::Point3(1., 0., 0.), util::Point3(0., 1., 0.),
                   util::Point3(0., 0., 0.), util::Point3(0., 0., 1.)};
          qds = quad.getQuadPoints(nodes);
          //
          // After changing the order of vertices, we have got a new
          // triangle which is in coordinate system (x,y) and we are
          ...
          // Thus the following method for I_approx is correct.
          debug_id = 1;
          if (!checkRefIntegration(n, i, j, k, qds, I_exact)) error_test_1++;

          //
          // when vertices are {(0,1,0), (0,0,1), (0,0,0), (1,0,0)}
          //
          nodes = {util::Point3(0., 1., 0.), util::Point3(0., 0., 1.),
                   util::Point3(0., 0., 0.), util::Point3(1., 0., 0.)};
          qds = quad.getQuadPoints(nodes);
          debug_id = 2;
          if (!checkRefIntegration(n, i, j, k, qds, I_exact)) error_test_1++;
          ...
          //
          nodes = {util::Point3(0., 0., 0.), util::Point3(0., 0., 1.),
                   util::Point3(1., 0., 0.), util::Point3(0., 1., 0.)};
          qds = quad.getQuadPoints(nodes);
          debug_id = 3;
          if (!checkRefIntegration(n, i, j, k, qds, I_exact)) error_test_1++;
        }
  ...
  size_t error_test_2 = 0;
  {
    ...
    static size_t num_elems = 0;
    if (num_elems == 0) {
      readNodes("tetMesh_nodes.csv", nodes);
      num_elems = readElements("tetMesh_elements.csv", elem_type, elements);
    }
            ...
            for (auto qd : qds) {
              ...
              if (false) {
                ...
                std::vector<size_t> enode_ids = {
                    elements[num_vertex * e + 0], elements[num_vertex * e + 1],
                    elements[num_vertex * e + 2], elements[num_vertex * e + 3]};
