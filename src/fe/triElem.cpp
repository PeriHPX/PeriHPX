...
std::vector<fe::QuadData> fe::TriElem::getQuadDatas(
...
std::vector<fe::QuadData> fe::TriElem::getQuadPoints(
    const std::vector<util::Point3> &nodes) {
  // copy quad data associated to reference element
  auto qds = d_quads;
...
util::Point3 fe::TriElem::mapPointToRefElem(
    const util::Point3 &p, const std::vector<util::Point3> &nodes) {
...
double fe::TriElem::getJacobian(const util::Point3 &p,
                                const std::vector<util::Point3> &nodes,
                                std::vector<std::vector<double>> *J) {
  if (J != nullptr) {
    J->resize(2);
