...
std::vector<fe::QuadData> fe::LineElem::getQuadDatas(
...
std::vector<fe::QuadData> fe::LineElem::getQuadPoints(
    const std::vector<util::Point3> &nodes) {
  // copy quad data associated to reference element
  auto qds = d_quads;
...
util::Point3 fe::LineElem::mapPointToRefElem(
    const util::Point3 &p, const std::vector<util::Point3> &nodes) {
  ...
  if (util::compare::definitelyLessThan(xi, -1. - 1.0E-8) ||
    ...
    std::cerr << "Error: Trying to map point p = " << p.d_x
              ...
              << "But the point p does not belong to line = {" << nodes[0].d_x
              << ", " << nodes[1].d_x << "}.\n";
    exit(1);
  }

  if (util::compare::definitelyLessThan(xi, -1.)) xi = -1.;
  if (util::compare::definitelyGreaterThan(xi, 1.)) xi = 1.;
