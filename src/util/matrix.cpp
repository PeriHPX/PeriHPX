...
bool util::checkMatrix(const std::vector<std::vector<double>> &m) {
  ...
  if (!check) check = row_size > 2 and col_size != m[2].size();
...
std::vector<double> util::dot(const std::vector<std::vector<double>> &m,
  ...
  for (size_t i = 0; i < row_size; i++)
    for (size_t j = 0; j < row_size; j++) r[i] += m[i][j] * v[j];
...
std::vector<std::vector<double>> util::inv(
  ...
  if (row_size == 1) {
    ...
    n[0][0] = det_inv * (m[1][1] * m[2][2] - m[2][1] * m[1][2]);
    n[0][1] = -det_inv * (m[0][1] * m[2][2] - m[2][1] * m[0][2]);
    n[0][2] = det_inv * (m[0][1] * m[1][2] - m[1][1] * m[0][2]);

    n[1][0] = -det_inv * (m[1][0] * m[2][2] - m[2][0] * m[1][2]);
    n[1][1] = det_inv * (m[0][0] * m[2][2] - m[2][0] * m[0][2]);
    n[1][2] = -det_inv * (m[0][0] * m[1][2] - m[1][0] * m[0][2]);

    n[2][0] = det_inv * (m[1][0] * m[2][1] - m[2][0] * m[1][1]);
    n[2][1] = -det_inv * (m[0][0] * m[2][1] - m[2][0] * m[0][1]);
    n[2][2] = det_inv * (m[0][0] * m[1][1] - m[1][0] * m[0][1]);
